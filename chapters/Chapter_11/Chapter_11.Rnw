...
<<label=LBL10, results="asis", echo=FALSE, out.width="10cm", out.height="6cm">>=
  xr <- seq(50, 90, by=0.1)
  yr <- dnorm(xr, mean=69, sd=3)
  df1<-data.frame(xr,yr)
  p<-ggplot(df1,aes(x=xr, y=yr))+geom_line() + theme_bw() +
    geom_area(data = subset(df1, xr > 66 & xr < 72), aes(x=xr, y=yr), fill="skyblue") +
    theme(panel.border=(element_blank())) +
    labs(x="Heights (inches)", y=NULL, size=3) + scale_y_continuous(breaks=NULL) + 
    scale_x_continuous(breaks=seq(60,78,3)) +
    annotate("text",x=80,y=0.1, label="0.6827", color="black", size=5) + 
    annotate("segment",x=76,xend=71, y=0.1,yend=0.1,color="black", size=0.5, arrow=arrow()) 
...
<<label=LBL11b, results="asis", echo=FALSE>>=
  MU   <- 9.02
  XBAR <- 8.10
  SD   <- 0.70
  N    <- 12
...
\begin{enumerate}
  \item Stays the same.
  \item $\bar{X} = 8.10$
  \item $ SE = \frac{0.70}{\sqrt{12}} = \Sexpr{SE1}$ 
\end{enumerate}
