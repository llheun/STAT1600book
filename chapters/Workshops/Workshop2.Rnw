...
%   \item difference: \Sexpr{mnDiff}
%   \item SE: \Sexpr{SDdiff} 
%   \item $Z_0 = \Sexpr{z2}$?
...
<<label=LBL18a, results="asis", echo=FALSE>>=
  red1   <- read.csv("data/winequalityRed.csv", sep = ";")
  white1 <- read.csv("data/winequalityWhite.csv", sep = ";")
  red1['color']   <- 'red'
  white1['color'] <- 'white'
  redAC    <- sample(red1$alcohol, size = 36)
  whiteAC  <- sample(white1$alcohol, size = 36)
  
  nwhite2  <- length(whiteAC)
  mnwhite2 <- mean(whiteAC) 
  sdwhite2 <- sd(whiteAC)
  ...
  mnred2 <- sprintf("%.3f", mnred2)
  sdred2 <- sprintf("%.4f", sdred2)
  
  mnwhite2 <- sprintf("%.3f", mnwhite2)
  sdwhite2 <- sprintf("%.4f", sdwhite2)
...
Suppose researchers decided to take a random sample of 36 from each type of wine.  The wine quality dataset used in this chapter includes $alcohol content$. The average $alcohol content$ of red wine is \Sexpr{mnred2}, while that of white wine is \Sexpr{mnwhite2}. The respective standard deviations are \Sexpr{sdred2} and \Sexpr{sdwhite2}. The respective sample sizes are \Sexpr{nred2} and \Sexpr{nwhite2}.
...
<<label=LBL18b, results="asis", echo=FALSE>>=
  # red1   <- read.csv("data/winequalityRed.csv", sep = ";")
  # white1 <- read.csv("data/winequalityWhite.csv", sep = ";")
  # red1['color']   <- 'red'
  # white1['color'] <- 'white'
  redQU    <- sample(red1$quality, size = 64)
  whiteQU  <- sample(white1$quality, size = 64)
  
  nwhite2  <- length(whiteQU)
  mnwhite2 <- mean(whiteQU) 
  sdwhite2 <- sd(whiteQU)
  nred2  <- length(redQU)
  mnred2 <- mean(redQU) 
  sdred2 <- sd(redQU)
  ...
  mnred2 <- sprintf("%.1f", mnred2)
  sdred2 <- sprintf("%.2f", sdred2)
  
  mnwhite2 <- sprintf("%.1f", mnwhite2)
  sdwhite2 <- sprintf("%.2f", sdwhite2)
...
<<label=LBL18c, results="asis", echo=FALSE>>=
  # red1   <- read.csv("data/winequalityRed.csv", sep = ";")
  # white1 <- read.csv("data/winequalityWhite.csv", sep = ";")
  # red1['color']   <- 'red'
  # white1['color'] <- 'white'
  redPH    <- sample(red1$pH, size = 100)
  whitePH  <- sample(white1$pH, size = 100)
  
  nwhite2  <- length(whitePH)
  mnwhite2 <- mean(whitePH) 
  sdwhite2 <- sd(whitePH)
