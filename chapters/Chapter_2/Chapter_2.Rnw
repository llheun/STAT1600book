...
An interval variable is similar to an ordinal variable, except that the intervals between the values of the interval variable are equally spaced.  For example, suppose we have a variable such as the annual income that we measure in dollars, and we have three people who make \$10,000, \$15,000 and \$20,000. The second person makes \$5,000 more than the first person and \$5,000 less than the third person, and the size of these intervals is the same.  If two other people make \$90,000 and \$95,000, the size of that interval between these two people is also the same (\$5,000).  \citep{bruin2011}
...
Ratio variables are numerical variables with true zero.  Many credit hours is a ratio measurement.  A student who has completed 90 credit hours has twice as many as 45 credit hours and three times as many as 30 credit hours.  Another example is the temperature on the Kelvin scale, which has true zero because there is no molecular motion.  A Kelvin temperature of 400 is twice as high as 200 Kelvin.
...
A \textit{bar chart} is a plot of the relative frequency table.  We use bar charts for categorical data.  Data scientists present a bar chart where the order of the categories in the bar chart is arbitrary.  Sometimes we present the graph %they are presented in order of 
in decreasing frequency for aesthetic purposes and is called a Pareto chart.  Some computing packages present the categories alphabetically unless otherwise specified.

<<label=LBL2b, results="asis", echo=FALSE>>=
  barplot(v2, main="Bar Plot for Type of Payment", xlab="Type of Monthly Payment", ylab="Relative Frequency", col="green")
@

\subsection{Pie Chart}

A pie chart gives the same information as a bar chart, except in a circular shape.  We also use pie charts for categorical data.   Some people prefer the look of pie charts over bar charts.  However, studies have shown that people have difficulty comparing the relative size of wedges.  When using this type of chart, we should only consider it when the number of variables is less than six.  Look at the pie chart below.  Is Rent less than half or more than half of Mortgage?

<<label=LBL2c, results="asis", echo=FALSE, out.width="7cm">>=
...
{\small{
<<label=LBL2e, results="asis", echo=FALSE>>=
  DT2s <- sort(DT2)
@  
...
{\footnotesize{
\begin{verbatim}
<<label=LBL2ee, results="asis", echo=FALSE>>=
  stem(x=DT2, scale=2, width=22) 
@ 
\end{verbatim}
}}
...
{\small{
<<label=LBL2f, results="asis", echo=FALSE>>=
...
<<label=LBL2g, results="asis", echo=FALSE, out.width="9cm">>=
   B1 <- c(68,60,76,68,64,80,72,76,92,68,56,72,68,60, 72,56,88,76,80,68,80,84,64,80,72,69,68,76)
   A1  <- B1 + rnorm(n=28, mean=10, sd=5)
   Pulse <- c(B1, A1)
   Time_ <- c(rep("Before", 28), rep("After", 28))
   
   bwidth <- 1
   dfrm <- data.frame(Pulse, Time_)
   ggplot(data=dfrm, aes(x = Time_, y =  Pulse, fill = Time_)) + 
...
<<label=LBL2h, results="asis", echo=FALSE, out.width='8cm'>>=
  boxplot(tmp0[13:75], horizontal=TRUE, xlab="Dollars", main="Box of Monthly Payments", col="green")
@
...
<<label=LBL2i, results="asis", echo=FALSE, out.width="9cm">>=
  boxplot(Monthly.Payment ~ Type.of.Payment, horizontal=TRUE, xlab="Dollars", main="boxplot of Monthly Payments", ylab="Type", col="green")
@
...
MED is the .50(n+1)st ordered observation. In other words, the median is the middle value or the average of the two middle values \newline
...
<<label=LBL2k, results="asis", echo=FALSE>>=
 SDat2 <- c(45,48,53,51,51,52,54,55,55,56,59,62,64,64,65,71,77,85,92,105,112)
...
<<label=LBL2l, results="asis", echo=FALSE>>=
   SDat3 <- c(40,55,62,75,81,87,92,94,94,95,101,101,101,102,104,105,105,106,109,115,118)
##   stem(SDat3, scale=2)
@
...
<<label=LBL2m, results="asis", echo=FALSE, out.width="4cm">>=
  SDat4 <- c(401,501,601,702,801,802,901,902,903,904,905,1001,1002,1003,1004,1005,1006,1007,1008,1101,1102)
  hist(SDat4, xlab="values", col="yellow")
@

A long tail denotes presence of extreme or outlying observations.  The left-skewed histogram above contains observations that are small and outlying, while a right-skewed histogram would have observations that are large and outlying.  A left skew also indicates that the smallest quarter of observations will be more spread out. For some further examples of histograms depicting both symmetry and skewness, see below.

\vspace{3mm}

\begin{figure}[ht]

<<label=LBL2n, resutls="asis", echo=FALSE, out.width="4cm">>=
...
\begin{itemize}
\item	We cannot use the \textit{mean} or \textit{median} for numerical variables.
