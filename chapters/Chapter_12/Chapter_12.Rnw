...
Statisticians will conclude `independence' if Tables 12.1 and 12.2 are close and conclude `association' if they are far from each other.   Closeness is and fairness is measured by subtraction and squaring, as follows:
