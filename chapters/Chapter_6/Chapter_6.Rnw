...
<<label=LBL6a, results="asis", echo=FALSE, out.width="7cm", out.height="4cm" >>=
dataset<-rnorm(n=1000000, mean=36, sd=8)
dataset <- data.frame(X=dataset)

hist1<-ggplot(dataset,aes(x=X)) +
        geom_histogram(binwidth=3,fill="white",color="black") +
        geom_histogram(data=subset(dataset,X>40.5),binwidth=3,
                 color="black", fill="skyblue")+ labs(y=NULL) + theme_bw() +
        scale_y_continuous(breaks=NULL) + theme(panel.border=(element_blank()))

density1<-ggplot(dataset,aes(x=X)) + geom_density(fill="white",color="black")+theme_bw() +
        labs(y=NULL) + scale_y_continuous(breaks=NULL) + theme(panel.border=(element_blank()))

d <- ggplot_build(density1)$data[[1]]
density1 <- density1 + geom_area(data = subset(d, x > 40.5), aes(x=x, y=y), fill="skyblue")

ggarrange(hist1, density1, ncol = 2, nrow = 1)
...
<<label=LBL6b, results="asis", echo=FALSE, out.width="8cm", out.height="7cm" >>=
  xv <- seq(from=-4, to=4, by=0.01)
  yv <- dnorm(xv, mean=0, sd=1)
  x_coor  <- seq(from=-4, to=4, by=1)
  z_labs  <- x_coor
  yrng  <- diff(range(yv))
  df1<-data.frame(xv,yv)

  p1<-ggplot(df1,aes(x=xv, y=yv))+geom_line() + 
        labs(y=NULL, x="Z-score") + theme_bw() + #scale_x_continuous(breaks=NULL) +
        scale_y_continuous(breaks=NULL) + theme(panel.border=(element_blank())) 
        #annotate(geom = "text", x=0, y=-0.20*yrng, label="Z-score", size=4) +
        #annotate(geom = "text", x=x_coor, y=-0.29*yrng, label=z_labs, size=4) +
        #annotate(geom = "text", x=x_coor, y=-0.25*yrng, label="|", size=3) 
  p1
@
\end{figure}
...
<<label=LBL6c1, results="asis", echo=FALSE, out.width="7cm", out.height="7cm">>=
  xr <- seq(-3, 3, by=0.01)
  yr <- dnorm(xr, mean=0, sd=1)
  x_coor  <- seq(from=-3, to=3, by=1)
  x_labs  <- seq(12,60,8)
  z_labs  <- x_coor
  yrng  <- diff(range(yr))
  df1<-data.frame(xr,yr)

  p1<-ggplot(df1,aes(x=xr, y=yr))+geom_line() + 
        geom_area(data = subset(df1, xr > -1 & xr < 1), aes(x=xr, y=yr), fill="skyblue") +
        labs(y=NULL, x=NULL) + theme_bw() + scale_x_continuous(breaks=NULL) +
        scale_y_continuous(breaks=NULL) + #theme(panel.border=(element_blank())) +
        annotate(geom = "text", x=0, y=-0.04*yrng, label="X", size=4) +
        annotate(geom = "text", x = x_coor, y = -0.12*yrng, label = x_labs, size=4) +
        annotate(geom = "text", x=0, y=-0.20*yrng, label="Z-score", size=4) +
        annotate(geom = "text", x=x_coor, y=-0.30*yrng, label=z_labs, size=4) +
        annotate(geom = "text", x=x_coor, y=-0.07*yrng, label="|", size=3) +
        annotate(geom = "text", x=x_coor, y=-0.25*yrng, label="|", size=3) +
        annotate("text",x=2,y=0.352, label="0.6827", color="black", size=5) + 
        annotate("segment",x=2,xend=0.9, y=0.33,yend=0.242,color="black", size=1, arrow=arrow()) 
  p1
...
\end{minipage}
\begin{minipage}[ht]{7cm}
<<label=LBL6c2, results="asis", echo=FALSE, out.width="7cm", out.height="7cm">>=
  xs <- seq(-3, 3, by=0.01)
  ys <- dnorm(xs, mean=0, sd=1)
  df1<-data.frame(xr,yr)

  p1<-ggplot(df1,aes(x=xr, y=yr))+geom_line() + 
        geom_area(data = subset(df1, xr > -2 & xr < 2), aes(x=xr, y=yr), fill="skyblue") +
        labs(y=NULL, x=NULL) + theme_bw() + scale_x_continuous(breaks=NULL) +
        scale_y_continuous(breaks=NULL) + #theme(panel.border=(element_blank())) +
        annotate(geom = "text", x=0, y=-0.04*yrng, label="X", size=4) +
        annotate(geom = "text", x = x_coor, y = -0.12*yrng, label = x_labs, size=4) +
        ...
        annotate(geom = "text", x=x_coor, y=-0.07*yrng, label="|", size=3) +
        annotate(geom = "text", x=x_coor, y=-0.25*yrng, label="|", size=3) +
        annotate("text",x=2,y=0.352, label="0.9545", color="black", size=5) + 
        annotate("segment",x=2,xend=1.3, y=0.33,yend=0.13,color="black", size=1, arrow=arrow()) 
  p1
...
<<label=LBL6d, results="asis", echo=FALSE, out.width="7cm", out.height="7cm" >>=
  xr <- seq(-3, 3, by=0.01)
  yr <- dnorm(xr, mean=0, sd=1)
  x_coor  <- seq(from=-3, to=3, by=1)
  x_coor <-c(x_coor,0.5)
  ...
  yrng  <- diff(range(yr))
  df1<-data.frame(xr,yr)
  p<-ggplot(df1,aes(x=xr, y=yr))+geom_line() + 
     geom_area(data = subset(df1, xr > 0.5), aes(x=xr, y=yr), fill="skyblue") +
     labs(x=NULL, y=NULL) + scale_y_continuous(breaks=NULL) + scale_x_continuous(breaks=NULL) +
     annotate(geom = "text", x=0, y=-0.04*yrng, label="X", size=4) +
     annotate(geom = "text", x = x_coor, y = -0.12*yrng, label = x_labs, size=4) +
     annotate(geom = "text", x=0, y=-0.18*yrng, label="Z-score", size=4) +
     annotate(geom = "text", x=x_coor, y=-0.28*yrng, label=z_labs, size=4) + theme_bw() +
     annotate(geom = "text", x=x_coor, y=-0.07*yrng, label="|", size=3) +
     annotate(geom = "text", x=x_coor, y=-0.23*yrng, label="|", size=3) +
...
<<label=LBL6e, results="asis", echo=FALSE, out.width="8cm", out.height="7cm" >>=
  xr <- seq(-3, 3, by=0.01)
  yr <- dnorm(xr, mean=0, sd=1)
  x_coor  <- c(-3, -2, -1, 0, 1.28, 2, 3)
  x_labs  <- c('12','20','28','36','a','52','60')
  z_labs  <- x_coor
  yrng  <- diff(range(yr))
  df1<-data.frame(xr,yr)
  p<-ggplot(df1,aes(x=xr, y=yr))+geom_line() + 
     geom_area(data = subset(df1, xr > 1.28), aes(x=xr, y=yr), fill="skyblue") +
     labs(x=NULL, y=NULL) + scale_y_continuous(breaks=NULL) + scale_x_continuous(breaks=NULL) +
     annotate(geom = "text", x=0, y=-0.04*yrng, label="X", size=4) +
     annotate(geom = "text", x = x_coor, y = -0.12*yrng, label = x_labs, size=4) +
     annotate(geom = "text", x=0, y=-0.18*yrng, label="Z-score", size=4) +
     ...
     annotate(geom = "text", x=x_coor, y=-0.07*yrng, label="|", size=3) +
     annotate(geom = "text", x=x_coor, y=-0.23*yrng, label="|", size=3) +
     annotate("text",x=2.5,y=0.15, label="10%", color="black", size=4) + 
     # annotate(geom = "text", x=0, y=+1.2*yrng, label="Figure 6.5 : The 90th Percentile", size=5) +
     annotate("segment",x=2.4,xend=2, y=0.12,yend=0.03,color="black", size=1, arrow=arrow()) +
     theme(panel.border=(element_blank()))
...
cell phone users concerning monthly usage.  How much time can you spend on your phone per day?
   ...
   % \framebox[5cm][l]{ Answer: }
	\end{exercise}
	\begin{solution}  % 1
<<label=LBL46-1, results='asis', echo=FALSE>>=
  MN1 <- 2.4
  SD1 <- 1.1
...
<<label=LBL6-3, result="asis", echo=FALSE>>=
  sd2 <- (35 - 4)/4
@
...
\end{enumerate}
  ...
  \end{exercise}
\begin{solution}  % 5
<<label=LBL6-5, results="asis", echo=FALSE>>=
  mn5 <- 42.14
  sd5 <- 1.43
...
<<label=LBL8i, results="asis", echo=FALSE>>=
  p9a <- sprintf("%.4f", pnorm(q=110, mean=138.5, sd=29))
  p9b <- sprintf("%.4f", 1 - pnorm(q=150, mean=138.5, sd=29))
  p9c <- sprintf("%.4f", pnorm(q=180, mean=138.5, sd=29) - pnorm(q=120, mean=138.5, sd=29))
  p9d <- sprintf("%.4f", 1 - pnorm(q=180, mean=138.5, sd=29))
@  
...
\begin{enumerate}
\item $P[ X < 110 ] = \Sexpr{p9a} $
\item $P[ X > 150 ] = \Sexpr{p9b} $
\item $P[ 120 < X < 180 ] = \Sexpr{p9c} $
\item $P[ X > 180 ] = \Sexpr{p9d} $ Yes.
\end{enumerate}
