...
\subsection{The Sampling Distribution of $\hat{p}$ is Approximately Normal}
...
\caption{Probability of Histogram and $\hat{p}$}
<<label=LBL8a, results="asis", echo=FALSE, out.width="15cm", out.height="10cm">>=
