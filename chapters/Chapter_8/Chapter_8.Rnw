...
<<label=LBL6a, results="asis", echo=FALSE, out.width="8cm">>=
...
\end{exercise}
\begin{solution} % 1
<<label=LBL6-1, results="asis", echo=FALSE>>=
  N1 <- 125
  Q1 <- 0.25 * 125
  P1 <- 0.20
  prob61 <- 1 - pbinom(q=Q1, size=N1, prob=P1)
@

$P[ X \ge 32 ] = \Sexpr{prob61}
\end{solution}
...
\end{exercise}
\begin{solution} % 3
<<label=LBL6-3, results="asis", echo=FALSE>>=
  N1 <- 100
  prob63a <- .2
  SE63b   <- sqrt(prob63a * (1 - prob63a) / N1)
  me63c   <- 1.96 * SE62b
@

The standard error of this estimate is \Sexpr{SE63b}
...
\end{exercise}
\begin{solution} % 6
<<label=LBL6-6, results="asis", echo=FALSE>>=
  N1 <- 5511
  X1 <- 1754
  prob66a <- X1 /N1
...
\end{exercise}
\begin{solution} % 7
<<label=LBL6-7, results="asis", echo=FALSE>>=
  N1 <- 2000
  X1 <- 600
  prob67a <- X1 /N1
