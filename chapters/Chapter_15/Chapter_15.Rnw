...
<<label=LBL13a, results="asis", echo=FALSE, out.width="10cm", out.height="6cm">>=
...
Figure 13.2 shows various scatterplots with different correlations.  Although $0.5$ is halfway between $0$ and $1$, note that the plot corresponding to $r = 0.5$ barely shows a pattern of association.  In practice, plots can show correlations up to $0.3$ purely by accident (e.g., the correlation between GPA and, say, shoe size).  When correlation reaches $+1$ or $-1$, all points fall on a straight line.
...
<<label=LBL13b, results="asis", echo=FALSE, out.width="5cm">>=
  plot(wt1, mpg1, xlab="Car weight", ylab="MPG")
@
<<label=LBL13c, results="asis", echo=FALSE, out.width="5cm">>=
  plot(rs1, pe1, xlab="R/S", ylab="P/E")
@
<<label=LBL13d, results="asis", echo=FALSE, out.width="5cm">>=
  plot(temp1, use1, xlab="Ave-Temp", ylab="Power Usage (KW)")
@  
<<label=LBL13e, results="asis", echo=FALSE, out.width="5cm">>=
